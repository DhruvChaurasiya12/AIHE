...
const CourseCard = ({ course, index, onRegister }: CourseCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl overflow-hidden hover:shadow-xl transition-all duration-500 border border-primary/5 flex flex-col group h-full"
    >
      ...
      <div className="px-4 py-2 sm:py-4 sm:px-6 flex flex-col flex-grow">
        <p className="text-muted-foreground text-sm leading-relaxed mb-4 line-clamp-2 italic">
          {description}
        </p>

        <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 bg-secondary/20 p-2 sm:p-4 rounded-2xl border border-primary/5">
          {/* Instructor */}
          <div className="flex items-center gap-3 col-span-2 border-b border-primary/10 pb-2">
        ...
        <div className="mt-auto flex  sm:flex-row gap-4 sm:items-center justify-between pt-4 border-t border-primary/5">
          <div className="flex flex-col">
            <span className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold">Course Fee</span>
            <div className="flex items-baseline gap-0.5 text-primary">
              <IndianRupee className="w-3.5 h-3.5" />
              <span className=" text-xl sm:text-2xl font-bold">{course.fee > 0 ? Number(course.fee).toLocaleString() : "Free"}</span>
            </div>
          </div>
          ...
          <Button 
            ...
            className={`w-ful sm:w-auto rounded-2xl px-6 sm:px-8 font-bold transition-all duration-300 ${
              isRegistrationOpen 
